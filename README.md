...
- **最近文件**：默认按**创建时间**排序，可点击“最近文件”右侧的排序按钮或在插件设置里切换为**修改时间**；展示数量、预览行数均可在设置中调整（默认 20 条、2 行预览）
- 排序时间优先读取笔记属性 `created` / `updated`（属性名可在设置中修改，支持 `2026-09-22T14:16+08:00`、`2026-09-22 14:16`、`2026-09-22` 等格式），缺失时回退到文件自身的创建/修改时间，避免同步工具重置文件时间导致排序错乱
